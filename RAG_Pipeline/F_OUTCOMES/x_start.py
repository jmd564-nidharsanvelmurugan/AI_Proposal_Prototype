...
# =====================================================
# Outcomes Template Schema
# =====================================================

class OutcomesSubSection(BaseModel):
    subsection_name: str
    query: str = ""


class OutcomesSection(BaseModel):
    section_name: str = "Outcomes"
    subsections: List[OutcomesSubSection]


class OutcomesTemplate(BaseModel):
...
outcomes_template_llm = llm.with_structured_output(
    OutcomesTemplate
)
...
# =====================================================
# Outcomes Template Generation Prompt
# =====================================================

outcomes_template_prompt = ChatPromptTemplate.from_template(
"""
You are an expert Proposal Architect.

Generate ONLY the Outcomes section structure.

The Outcomes section should have subsections that logically group the outcomes and KPIs.

Based on the questionnaire, common outcomes categories include:

1. Business Outcomes (e.g., Revenue Growth, Cost Reduction)
2. Key Performance Indicators (e.g., L2A Conversion, CAC)
3. Success Metrics (e.g., Data Accuracy, Report Refresh Time)
4. Risk Mitigation (e.g., Data Quality, Integration Risks)
...
- Capture the overall business meaning of outcomes expected.
- Maximum 15 words.
...
outcomes_template_chain = (
    outcomes_template_prompt
    | outcomes_template_llm
)
...
# =====================================================
# EMPTY PLACEHOLDERS (only these 4)
# =====================================================

problem_statement_placeholder = ""  # Empty - will be filled by Problem Statement module
objectives_placeholder = ""  # Empty - will be filled by Objectives module
deliverables_placeholder = ""  # Empty - will be filled by Deliverables module
approach_placeholder = ""  # Empty - will be filled by Approach module
...
save_to_json(metadata_dict, f"outcomes_metadata_{timestamp}.json")
...
save_to_json(top_proposals, f"outcomes_top_proposals_{timestamp}.json")
...
parent_chunks = get_parent_chunks_by_document_ids(document_ids, "Outcomes")
...
save_to_json({"parent_chunks": parent_chunks_safe, "child_chunks_count": len(child_chunks)}, f"outcomes_chunks_{timestamp}.json")


# =====================================================
# Step 4 - Generate Outcomes Template
# =====================================================

print("\n" + "=" * 60)
print("STEP 4: Generating Outcomes Template")
print("=" * 60)

outcomes_template = outcomes_template_chain.invoke(
    {
        "questionnaire": questionnaire,
...
template_dict = outcomes_template.model_dump()
print("\n✅ Outcomes template generated:")
print(json.dumps(template_dict, indent=2))

save_to_json(template_dict, f"outcomes_template_{timestamp}.json")


# =====================================================
# Step 5 - Extract KPI Metrics
# =====================================================

print("\n" + "=" * 60)
print("STEP 5: Extracting KPI and Success Metrics")
print("=" * 60)

kpi_metrics = extract_kpi_metrics(questionnaire)
print(f"\n📊 KPI Metrics Extracted:")
print(f"   - KPIs: {kpi_metrics['kpis']}")
print(f"   - Business Outcomes: {kpi_metrics['business_outcomes']}")
print(f"   - Success Metrics: {kpi_metrics['success_metrics']}")
print(f"   - Risk Mitigations: {kpi_metrics['risk_mitigations']}")

save_to_json(kpi_metrics, f"outcomes_kpi_metrics_{timestamp}.json")


# =====================================================
# Step 6 - User Interaction: Include Subsections?
# =====================================================

print("\n" + "=" * 60)
print("STEP 6: User Interaction - Subsection Selection")
print("=" * 60)
...
if include_subsections == "yes":
    ...
    for i, subsection in enumerate(outcomes_template.outcomes.subsections, 1):
        print(f"   {i}. {subsection.subsection_name}")
    
    choice = input("\nDo you want to include (a)ll subsections or (s)pecific ones? (a/s): ").strip().lower()
    
    if choice in ["a", "all"]:
        subsections_to_include = outcomes_template.outcomes.subsections.copy()
        ...
        if selection:
            ...
            for idx in selected_indices:
                if 0 <= idx < len(outcomes_template.outcomes.subsections):
                    subsections_to_include.append(outcomes_template.outcomes.subsections[idx])
...
"""
    ...
    print("\n" + "=" * 60)
    print("STEP 7: Filtering Chunks Based on Subsections")
    print("=" * 60)
    ...
    if use_query_filter == "yes":
        print("\n📝 Generating semantic query for the entire Outcomes section...")
        
        section_query = generate_section_query(questionnaire, metadata_dict, "Outcomes")
        print(f"\n   ✅ Generated Query: {section_query}")
        
        print("\n" + "=" * 60)
        print("STEP 7: Filtering Chunks with Semantic Query")
        print("=" * 60)
        ...
        print("\n" + "=" * 60)
        print("STEP 7: Fetching All Chunks (No Filtering)")
        print("=" * 60)
...
save_to_json(filtered_results, f"outcomes_filtered_chunks_{timestamp}.json")


# =====================================================
# Step 8 - Generate Outcomes Content
# =====================================================

print("\n" + "=" * 60)
print("STEP 8: Generating Outcomes Content")
print("=" * 60)

outcomes_content = generate_outcomes_content(
    questionnaire=questionnaire,
...
response = {
    ...
    "retrieved_chunks": filtered_results,
    "generated_content": outcomes_content,
    "user_preferences": {
...
if outcomes_content:
    content_file = os.path.join("x_results", f"generated_outcomes_{timestamp}.txt")
    with open(content_file, "w", encoding="utf-8") as f:
        f.write(outcomes_content)
    print(f"✅ Generated content saved to: {content_file}")
else:
    print("⚠️ No content generated")

print("\n" + "=" * 60)
print("GENERATED OUTCOMES PREVIEW")
print("=" * 60)
if outcomes_content:
    preview = outcomes_content[:600] + "..." if len(outcomes_content) > 600 else outcomes_content
    print(preview)
else:
...
save_to_json(response, f"outcomes_final_response_{timestamp}.json")
...
print(f"   - KPIs extracted: {len(kpi_metrics['kpis'])}")
print(f"   - Business outcomes extracted: {len(kpi_metrics['business_outcomes'])}")
print(f"   - Success metrics extracted: {len(kpi_metrics['success_metrics'])}")
if include_subsections == "yes":
...
print(f"   - Content generated: {'Yes' if outcomes_content else 'No'}")
if outcomes_content:
    print(f"   - Content length: {len(outcomes_content)} characters")

print("\n📁 Output files saved in 'x_results' folder:")
print(f"   - outcomes_metadata_{timestamp}.json")
print(f"   - outcomes_top_proposals_{timestamp}.json")
print(f"   - outcomes_chunks_{timestamp}.json")
print(f"   - outcomes_template_{timestamp}.json")
print(f"   - outcomes_kpi_metrics_{timestamp}.json")
print(f"   - outcomes_filtered_chunks_{timestamp}.json")
print(f"   - generated_outcomes_{timestamp}.txt")
print(f"   - outcomes_final_response_{timestamp}.json")
