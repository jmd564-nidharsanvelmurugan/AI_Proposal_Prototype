...
# Reuse the same retrieval helpers (they accept a section name)
...
class ProblemStatementSubSection(BaseModel):
    subsection_name: str
    query: str = ""


class ProblemStatementSection(BaseModel):
    section_name: str = "Problem Statement"
    subsections: List[ProblemStatementSubSection]


class ProblemStatementTemplate(BaseModel):
    problem_statement: ProblemStatementSection


# =====================================================
# LLM setup (reuse same Groq instance)
# =====================================================
...
problem_statement_llm = llm.with_structured_output(ProblemStatementTemplate)
...
# =====================================================
# Problem Statement Template Generation Prompt
# =====================================================

problem_statement_prompt = ChatPromptTemplate.from_template("""
You are an expert Proposal Architect.

Generate ONLY the Problem Statement section structure.

The Problem Statement section MUST contain these 3 subsections:

1. Current Challenges (or Pain Points)
2. Root Causes (or Underlying Issues)
3. Implications (or Business Consequences)
...
# =====================================================
# Generate Section Query (semantic search for the whole section)
# =====================================================

def generate_section_query(questionnaire: str, metadata: dict, section_name: str) -> str:
    """Generate a semantic retrieval phrase for the entire Problem Statement section."""
    query_prompt = f"""
...
- Focus on the key problems, pain points, and challenges described in the questionnaire.
- Include relevant business concepts and affected areas.
- Maximum 15 words.
- Suitable for semantic vector retrieval against child chunks.

Example for Problem Statement:
"fragmented financial data manual reconciliation reporting delays"
...
problem_statement_chain = problem_statement_prompt | problem_statement_llm
...
SECTION_NAME = "Problem Statement"   # <-- critical change
...
# =====================================================
# Step 3 – Fetch parent and child chunks for the Problem Statement section
# =====================================================
...
# =====================================================
# Step 4 – Generate Problem Statement Template (subsections)
# =====================================================

print("\n" + "=" * 60)
print("STEP 4: Generating Problem Statement Template")
print("=" * 60)

problem_template = problem_statement_chain.invoke({
    "questionnaire": questionnaire,
    "metadata": metadata.model_dump_json()
})

template_dict = problem_template.model_dump()
print("\n✅ Problem Statement template generated:")
print(json.dumps(template_dict, indent=2))
save_to_json(template_dict, f"problem_statement_template_{timestamp}.json")
...
if include_subsections == "yes":
    ...
    for i, sub in enumerate(problem_template.problem_statement.subsections, 1):
        print(f"   {i}. {sub.subsection_name}")

    choice = input("\nDo you want to include (a)ll subsections or (s)pecific ones? (a/s): ").strip().lower()

    if choice in ["a", "all"]:
        subsections_to_include = problem_template.problem_statement.subsections.copy()
        ...
        if selection:
            ...
            for idx in indices:
                if 0 <= idx < len(problem_template.problem_statement.subsections):
                    subsections_to_include.append(problem_template.problem_statement.subsections[idx])
...
- Do NOT use quotes.
- Capture the business meaning of the problem.
- Maximum 12 words.
...
"""
    ...
    if use_query_filter == "yes":
        print("\n📝 Generating semantic query for the entire Problem Statement section...")
...
# =====================================================
# Step 7 – Generate Problem Statement Content
# =====================================================

print("\n" + "=" * 60)
print("STEP 7: Generating Problem Statement Content")
print("=" * 60)

# Import the problem‑specific content generator (see separate file)
from c_generation import generate_problem_statement_content

problem_content = generate_problem_statement_content(
    questionnaire=questionnaire,
...
response = {
    ...
    "retrieved_chunks": filtered_results,
    "generated_content": problem_content,
    "user_preferences": {
...
if problem_content:
    content_file = os.path.join("x_results", f"generated_problem_statement_{timestamp}.txt")
...
response_summary = {
    ...
    "problem_statement_subsections": response["problem_statement"]["subsections"],
    "section_query_used": response["problem_statement"].get("section_query"),
...
print(f"   - Content generated: {'Yes' if problem_content else 'No'}")

print("\n📁 Output files saved in 'x_results' folder:")
for fname in ["metadata", "top_matching_proposals", "chunks", "problem_statement_template",
              "filtered_chunks", "generated_problem_statement", "final_response"]:
    print(f"   - {fname}_{timestamp}.json" if fname != "generated_problem_statement" else f"   - {fname}_{timestamp}.txt")
