...
from b_subsection_filter import (
    ...
    get_filtered_chunks_by_semantic_query  # New function
)
...
- Capture the overall business meaning and purpose of the engagement.
- Include key business concepts from both questionnaire and metadata.
- Maximum 15 words.
- Suitable for semantic vector retrieval against child chunks.

Example for Business Context:
"financial advisory wealth management reporting transformation executive visibility"
...
# Flag to track if we're using query-based filtering
...
if include_subsections == "yes":
    ...
    # Set subsections_list and proceed normally
    subsections_list = []
    generate_queries = "no"
...
"""
    ...
    # =====================================================
    # NO SUBSECTIONS SELECTED - New Flow
    # =====================================================
    print("\n⚠️ No subsections selected. Proceeding with section-level filtering...")
    
    # Ask user if they want query-based filtering
    use_query_filter = input("\n🔍 Do you want to use query-based semantic filtering? (yes/no): ").strip().lower()
    
    if use_query_filter == "yes":
        print("\n📝 Generating semantic query for the entire Business Context section...")
        
        # Generate a semantic-rich query from questionnaire and metadata
        section_query = generate_section_query(questionnaire, metadata_dict, "Business Context")
        ...
        # Filter chunks using the generated query
        ...
        # Store results in a special structure
        subsections_list = []
        generate_queries = "yes"  # Treated as query generated
        ...
        # Get all chunks without any filtering
        filtered_results = get_filtered_chunks_for_section(
            child_ids=all_child_ids,
            subsections=[],  # Empty list triggers "all_chunks" mode
            search_type=1,
...
# =====================================================
# Step 6 - Generate Business Context Content
# =====================================================

print("\n" + "=" * 60)
print("STEP 6: Generating Business Context Content")
print("=" * 60)
...
response_summary = {
    ...
    "section_query_used": response["business_context"].get("section_query"),
