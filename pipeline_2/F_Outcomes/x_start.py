...
def get_latest_section_file(section_name: str) -> Optional[str]:
    folder_map = {
        ...
        "Approach": ("../E_Approach/x_results", "approach"),
    }
...
def main():
    ...
    # Load only Objectives, Deliverables, Approach
    previous_sections = {}
    for section_name in ["Objectives", "Deliverables", "Approach"]:
    ...
    with open(f"x_results/outcomes_{timestamp}.txt", "w", encoding="utf-8") as f:
        f.write(content)
    print(f"\n✅ Saved to x_results/outcomes_{timestamp}.txt")
